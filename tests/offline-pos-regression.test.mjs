...
async function run() {
  ...
  assert.match(database, /depletion_qty: Math\.max\(1, Number\(item\.depletion_qty \|\| 1\)\)/)
  assert.match(database, /inventory_item_id: i\.inventory_item_id \|\| null/)
  assert.match(database, /depletion_qty: Math\.max\(1, Number\(i\.depletion_qty \|\| 1\)\)/)
  ...
  assert.match(layout, /<Clock size=\{10\} \/> \{pendingCount\}/)
  assert.match(layout, /<AlertCircle size=\{10\} \/> \{failedCount\}/)
