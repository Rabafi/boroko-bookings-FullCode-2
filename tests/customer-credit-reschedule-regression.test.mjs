...
async function run() {
  ...
  assert.match(bookings, /\.rpc\('create_multi_room_booking'/, 'Multi-room accommodation must use one atomic server RPC')
  assert.doesNotMatch(bookings, /await createBooking\(\{[\s\S]*room_id: plan\.room_id/, 'Multi-room accommodation must not create partial per-room client operations')
  assert.match(bookings, /\.rpc\('reschedule_accommodation_booking'/, 'rescheduleBooking must use the accommodation-aware RPC')
