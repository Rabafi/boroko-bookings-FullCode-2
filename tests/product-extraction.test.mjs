...
test('product route allowlist blocks cross-product modules', () => {
  ...
  assert.equal(isProductRouteAllowed('/documents', 'hospitality-pos'), false)
  ...
  assert.equal(isProductRouteAllowed('/documents', 'hotel'), true)
  ...
  assert.equal(isProductRouteAllowed('/documents', 'lodge-camp'), false)
...
test('Electron window captions use complete product names instead of property acronyms', () => {
  ...
  const windowOptions = main.slice(main.indexOf('function createWindow()'), main.indexOf('// -- Proactive Ops AI watcher'))
  assert.match(windowOptions, /title: APP_WINDOW_TITLE/)
  assert.match(windowOptions, /frame: true/)
  assert.match(windowOptions, /titleBarStyle: 'default'/)
  assert.doesNotMatch(windowOptions, /titleBarOverlay|titleBarStyle: 'hidden'/)
  assert.doesNotMatch(app, /app-window-titlebar|document\.body\.appendChild\(titlebar\)/)
  assert.doesNotMatch(css, /app-window-titlebar/)
  const hposCss = fs.readFileSync(path.join(root, 'src/renderer/src/styles/hospitality-pos.css'), 'utf8')
  assert.doesNotMatch(hposCss, /has-app-window-titlebar|100dvh - 36px/)
