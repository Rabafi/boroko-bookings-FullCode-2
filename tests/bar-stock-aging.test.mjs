...
test('Desktop Bar stock page exposes server age and remains honest offline', () => {
  ...
  assert.match(main, /inventory:getItemsWithReadStatus/)
  assert.match(main, /items: Array\.isArray\(rows\) \? rows : \[\]/)
  assert.match(main, /source: rows\?\._source \|\| 'unknown'/)
  assert.match(main, /complete: rows\?\._complete === true/)
  assert.match(main, /requireCapability\('inventory\.view'\)/)
  assert.match(preload, /getBarStockAging: \(outletId\) => (?:invoke|ipcRenderer\.invoke)\('inventory:getBarStockAging', outletId \|\| null\)/)
  assert.match(preload, /getItemsWithReadStatus: \(\) => (?:invoke|ipcRenderer\.invoke)\('inventory:getItemsWithReadStatus'\)/)
  assert.match(stock, /getBarStockAging/)
  assert.match(stock, /getItemsWithReadStatus/)
  assert.match(stock, /const inventoryRead = itemsResult\.value \|\| \{\}/)
  assert.match(stock, /setItemsRead\(\{ source: inventoryRead\.source \|\| 'unknown', complete: inventoryRead\.complete === true \}\)/)
  ...
  assert.match(stock, /outlets\.some\(\(outlet\) => String\(outlet\.id\) === String\(newItem\.outlet_id\)\)/)
  assert.match(stock, /outlet_id: durableOutletId/)
  ...
  assert.match(stock, /Offline — showing last saved stock\. Reconnect and refresh before counting\./)
