...
async function run() {
  ...
  assert.match(database, /const timeoutMs = Number\(options\?\.timeoutMs\) > 0 \? Number\(options\.timeoutMs\) : CONNECTIVITY_PROBE_TIMEOUT_MS/)
  assert.match(database, /setTimeout\(\(\) => ctrl\.abort\(\), timeoutMs\)/)
