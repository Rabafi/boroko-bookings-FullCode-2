...
-- Compatibility columns expected by the desktop cache refresh and manager PWA.
-- These are non-authoritative metadata fields; financial mutations still go
-- through RPCs.
...
-- Day-use structural fields that were introduced after the baseline schema.
-- Keep the existing total/deposit/balance model instead of adding independent
-- amount_paid/payment_status columns outside a payment ledger.
alter table public.pool_day_use
  ...
  add column if not exists package_fee numeric not null default 0,
...
update public.pool_day_use
   set updated_at = coalesce(updated_at, created_at, now()),
       base_total = coalesce(nullif(base_total, 0), total, 0),
       deposit_amount = case
         when coalesce(deposit_amount, 0) = 0
          and coalesce(balance_due, 0) = 0
          and coalesce(total, 0) > 0 then total
         else coalesce(deposit_amount, 0)
       end,
       balance_due = case
         when coalesce(deposit_amount, 0) = 0
          and coalesce(balance_due, 0) = 0
          and coalesce(total, 0) > 0 then 0
         else greatest(coalesce(total, 0) - coalesce(deposit_amount, 0), 0)
       end
 where updated_at is null
    or base_total = 0
...
begin
  if not exists (
    select 1
      from pg_constraint
     where conname = 'pool_day_use_status_check'
       and conrelid = 'public.pool_day_use'::regclass
  ) then
    alter table public.pool_day_use
      add constraint pool_day_use_status_check
      check (status in ('reserved', 'checked_in', 'active', 'completed', 'cancelled'));
  end if;

  if not exists (
    select 1
      from pg_constraint
     where conname = 'pool_day_use_pricing_mode_check'
       and conrelid = 'public.pool_day_use'::regclass
  ) then
    alter table public.pool_day_use
      add constraint pool_day_use_pricing_mode_check
      check (pricing_mode in ('per_person', 'flat', 'hourly', 'package'));
  end if;
end
$$;
...
-- Re-apply report RPC grants here so this compatibility migration fixes
-- production databases that missed the earlier grant-only migration.
