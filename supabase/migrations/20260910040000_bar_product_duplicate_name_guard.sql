-- Duplicate-name guard for the Bar product wizard contract.
--
-- Why: each failed wizard save minted its own operation key, so a retry
-- storm after an outage could create several same-named products/stock
-- items (operation-key dedupe cannot see cross-key duplicates). The guards
-- below fail closed BEFORE any mutation, with a recovery hint, and are
-- skipped for same-key replays (those returned earlier) and for the row
-- being edited. Derived bar_pack rows are excluded from the product match
-- since their names are generated. The queue/flow layers treat these
-- refusals as terminal (resolved-elsewhere), never as retryable failures.
