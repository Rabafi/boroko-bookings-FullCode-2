-- POS certified-history timeout fix, part 2 (read-path only; no data change).
--
-- Part 1 (20260915010000) removed the per-line movement scans, but monthly
-- ranges still time out: the itemized CTE runs one correlated jsonb_agg
-- subquery PER ORDER (thousands of sequential subqueries inside a single
-- statement) plus a giant single-statement jsonb assembly. This rewrite
-- aggregates the period's lines in ONE grouped pass and left-joins it per
-- order. Output shape, control totals, certification semantics, the auth
-- wrapper and grants are untouched (CREATE OR REPLACE preserves ACLs; this
-- file contains no GRANT/REVOKE).

begin;

-- Belt-and-braces for fresh chains (already live from part 1 otherwise).
create index if not exists pos_order_items_order_lodge_idx
  on public.pos_order_items (order_id, lodge_id);

create index if not exists inventory_movements_lodge_reference_item_idx
  on public.inventory_movements (lodge_id, reference_id, item_id);

create or replace function public.get_pos_financial_report_export_v2_unscoped(
  p_lodge_id uuid,
  p_start_date date,
  p_end_date date,
...
begin
  ...
  with filtered as (
    ...
    -- One pass over the period's depletion movements instead of a correlated
    -- scan per order line. NULL-item movements never matched the old per-line
    -- predicate (m.item_id = NULL is never true), so they stay excluded to
    -- keep results identical.
    select m.reference_id as order_id, m.item_id as item_id, sum(abs(m.total_cost)) as cost
      from public.inventory_movements m
     where m.lodge_id = p_lodge_id
       and m.movement_type in ('recipe_sale', 'sale', 'pos_sale', 'pos_return')
       and m.item_id is not null
       and m.reference_id in (select f.id from filtered f)
     group by m.reference_id, m.item_id
  ), order_items_agg as (
    -- One grouped pass over the period's lines instead of a correlated
    -- jsonb_agg subquery per order. to_jsonb(i) still serializes only the
    -- pos_order_items row (joined cost columns stay outside it), line order
    -- stays by item id, and orders without lines still resolve to '[]'.
    select i.order_id as order_id,
      jsonb_agg(to_jsonb(i) || jsonb_build_object(
        'gross', i.gross_subtotal,
        'discount', i.discount_allocated,
        'tax', i.tax_allocated,
        'net', coalesce(i.net_subtotal, i.subtotal),
        'cost', mc.cost
      ) order by i.id) as item_rows
      from public.pos_order_items i
      join filtered f on f.id = i.order_id
      left join movement_costs mc on mc.order_id = i.order_id and mc.item_id = i.inventory_item_id
     where i.lodge_id = p_lodge_id
     group by i.order_id
  ), itemized as (
    select c.*,
      coalesce(a.item_rows, '[]'::jsonb) as item_rows
    from classified c
    left join order_items_agg a on a.order_id = c.id
  ), tenderized as (
