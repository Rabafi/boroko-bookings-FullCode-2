-- Command Central: product-aware subscription management for the three-app split.
--
-- This is the direct super-admin path. It deliberately uses the same active
-- commercial catalogue as customer quote/activation flows, applies package
-- feature boundaries atomically, and records an activation audit entry.
...
begin
  ...
  for update;
  if not found then
    return jsonb_build_object('success', false, 'error', 'Company settings were not found');
  end if;
  ...
  if public.resolve_product_family(v_property_type) <> v_product_id then
    return jsonb_build_object(
      'success', false,
      'error', 'The selected app family does not match this company type',
      'expected_product_id', public.resolve_product_family(v_property_type)
    );
  end if;
  v_operating_profile := nullif(btrim(coalesce(v_settings.operating_profile->>'hospitality_mode', p_payload->>'operating_profile', '')), '');
  if v_product_id = 'hospitality-pos' and v_operating_profile is null then
    v_operating_profile := 'restaurant_bar';
  end if;
  ...
  if not found then
    return jsonb_build_object('success', false, 'error', 'The selected package is not in the active commercial catalogue');
  end if;
  ...
  if v_license_id is not null then
    ...
    for update;
    if not found then
      return jsonb_build_object('success', false, 'error', 'Selected license does not belong to the selected company');
    end if;
  else
    select * into v_license
    from public.licenses
  ...
  if v_license.id is null then
    ...
    update public.licenses
    set lodge_name = coalesce(nullif(p_payload->>'lodge_name', ''), lodge_name),
  ...
  -- Reset only catalogue-managed rows. Explicit Command Central overrides have
  -- their own reasons and remain in force until the operator clears them.
  for v_feature_key in
    ...
    from (
      ...
      where p.catalog_version_id = v_catalog.id
        and p.product_id in (v_product_id, coalesce(v_previous_product, v_product_id))
      union
      ...
      where a.catalog_version_id = v_catalog.id
        and a.product_id in (v_product_id, coalesce(v_previous_product, v_product_id))
    ) managed_features
  loop
  ...
  for v_feature_key in
    select feature_key
    from public.commercial_package_entitlements
    where catalog_version_id = v_catalog.id
      and product_id = v_product_id
      and commercial_package_key = v_package_key
  loop
    ...
    on conflict (lodge_id, feature_name) do update
      set enabled = true, reason = excluded.reason, updated_at = now()
      where public.lodge_features.reason is null
         or public.lodge_features.reason like 'Commercial package%'
         or public.lodge_features.reason like 'Commercial add-on%';
  end loop;
  ...
  for v_feature_key in
    ...
    where a.catalog_version_id = v_catalog.id
      ...
      and a.addon_key in (select jsonb_array_elements_text(v_addon_keys))
      and a.active = true
  loop
    insert into public.lodge_features (lodge_id, feature_name, enabled, reason, granted_at, updated_at)
  ...
  select coalesce(jsonb_object_agg(feature_name, enabled), '{}'::jsonb)
  into v_effective
