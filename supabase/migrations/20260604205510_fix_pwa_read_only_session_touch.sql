-- Keep read helpers side-effect free.
--
-- These functions are used from RLS policies and read-only RPCs such as the
-- Manager PWA dashboard/reports snapshots. Updating heartbeat or entitlement
-- metadata inside them can raise "cannot execute UPDATE in a read-only
-- transaction" when PostgREST runs a read request in a read-only transaction.
