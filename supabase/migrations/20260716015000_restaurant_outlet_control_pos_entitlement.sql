-- Restaurant & Bar outlet setup is a core POS configuration.  The original
-- multi-outlet enterprise add-on remains applicable to cross-property and
-- enterprise controls, not to creating this business's restaurant/bar scopes.
...
begin
  perform public.app_require_feature(p_lodge_id, 'pos', array['manager', 'admin', 'super_admin']);
  return query select o.id, o.name, o.type, o.is_active, o.sort_order, o.created_at, o.updated_at
    from public.outlets o where o.lodge_id = p_lodge_id
...
begin
  ...
  values (p_lodge_id, v_name, v_type, true, v_sort_order, v_actor_id, v_actor_id, now()) returning * into v_outlet;
...
returns jsonb language plpgsql security definer set search_path = public as $$
declare
  v_actor_id uuid := public.app_current_user_id(); v_before public.outlets%rowtype; v_after public.outlets%rowtype;
  v_name text := nullif(btrim(coalesce(p_payload->>'name', '')), ''); v_type text := lower(nullif(btrim(coalesce(p_payload->>'type', '')), ''));
  v_is_active boolean := coalesce((p_payload->>'is_active')::boolean, true); v_sort_order integer := coalesce(nullif(p_payload->>'sort_order', '')::integer, 0);
  v_active_count integer; v_action text;
begin
  perform public.app_require_feature(p_lodge_id, 'pos', array['manager', 'admin', 'super_admin']);
  ...
  if v_before.is_active and not v_is_active and v_active_count <= 1 then raise exception 'Keep at least one active outlet for this business'; end if;
  update public.outlets set name = v_name, type = v_type, is_active = v_is_active, sort_order = v_sort_order, updated_by = v_actor_id, updated_at = now()
  where id = p_outlet_id and lodge_id = p_lodge_id returning * into v_after;
