-- POS certified-history statement-timeout fix (read-path only; no data change).
--
-- get_pos_financial_report_export_v2_unscoped timed out on busy bars once the
-- monthly range grew (bulk offline runs: thousands of orders). Two hot spots:
--   1. each order line's stock cost ran a correlated subquery over
--      inventory_movements (a sequential scan per line without a covering
--      index), and
--   2. the per-order item probes on pos_order_items had no (order_id,
--      lodge_id) index.
-- The Sales history then fell back to provisional with
-- "canceling statement due to statement timeout".
--
-- Fix: pre-aggregate depletion costs once for the period's orders and add the
-- two covering indexes. Output shape, control totals, certification
-- semantics, the auth wrapper and grants are untouched (CREATE OR REPLACE
-- preserves ACLs; this file contains no GRANT/REVOKE).
  ...
  with filtered as (
    ...
    select c.*,
      coalesce((select jsonb_agg(to_jsonb(i) || jsonb_build_object(
        ...
        'cost', mc.cost
      ) order by i.id) from public.pos_order_items i left join movement_costs mc on mc.order_id = i.order_id and mc.item_id = i.inventory_item_id where i.order_id = c.id and i.lodge_id = p_lodge_id), '[]'::jsonb) as item_rows
    from classified c
  ), tenderized as (
