-- Repair ambiguous add_folio_charge / add_folio_payment overloads after idempotency migration.
-- charge_to_corporate_account calls add_folio_payment with 4 args; dual overloads broke resolution.

begin;

-- Drop all known signatures so a single defaulted function remains.
drop function if exists public.add_folio_charge(uuid, uuid, numeric, text, text, uuid);
drop function if exists public.add_folio_charge(uuid, uuid, numeric, text, text, uuid, text);
drop function if exists public.add_folio_payment(uuid, uuid, numeric, text);
drop function if exists public.add_folio_payment(uuid, uuid, numeric, text, text);
