-- Make card approval/reference optional on the restaurant bar POS.
--
-- Mobile-money tenders still require a provider transaction reference.
-- Card tenders keep an optional reference: when a reference is supplied it
-- must be 120 characters or fewer, but a card tender without a reference is
-- valid. Tender allocation integrity is preserved: a card payment_method
-- still requires a matching card row, and a mobile-money payment_method
-- still requires a matching mobile-money row with a reference.
begin;
...
begin
  if v_default_method = 'mobile_money'
     and jsonb_array_length(v_breakdown) = 0 then
    raise exception using
      errcode = '22023',
      message = 'mobile money tender requires a transaction or approval reference';
  end if;
