-- Rejected Online Bookings Log
-- ─────────────────────────────────────────────────────────────────────────────
-- Creates the rejected_online_bookings table and updates create_online_booking
-- to insert a rejection record whenever a booking attempt is blocked due to
-- maintenance or date conflicts.

begin;

-- 1. Table
create table if not exists public.rejected_online_bookings (
...
create index if not exists rejected_online_bookings_lodge_idx
  on public.rejected_online_bookings (lodge_id, attempted_at desc);
create index if not exists rejected_online_bookings_room_idx
  on public.rejected_online_bookings (lodge_id, room_id, attempted_at desc);

-- 2. RLS
alter table public.rejected_online_bookings enable row level security;
create policy "lodge_read_rejected_bookings"
  on public.rejected_online_bookings for select
  using (
    exists (
...
-- 3. Recreate create_online_booking with rejection logging
...
begin
  ...
  -- Catch maintenance exception so we can log the blocked attempt before returning
  begin
    perform public.app_check_room_maintenance(v_lodge_id, v_room_id);
  exception
    when others then
      insert into public.rejected_online_bookings
        (lodge_id, room_id, rejection_reason, guest_email, guest_name, check_in, check_out)
      values
        (v_lodge_id, v_room_id, 'maintenance',
         nullif(v_email, ''),
         nullif(btrim(v_first_name || ' ' || v_last_name), ''),
         v_check_in, v_check_out);
      return jsonb_build_object('success', false, 'error', 'Room is currently under maintenance and cannot be booked.');
  end;
  ...
  if v_conflict is not null then
    insert into public.rejected_online_bookings
      (lodge_id, room_id, rejection_reason, guest_email, guest_name, check_in, check_out)
    values
      (v_lodge_id, v_room_id, 'dates_taken',
       nullif(v_email, ''),
       nullif(btrim(v_first_name || ' ' || v_last_name), ''),
       v_check_in, v_check_out);
