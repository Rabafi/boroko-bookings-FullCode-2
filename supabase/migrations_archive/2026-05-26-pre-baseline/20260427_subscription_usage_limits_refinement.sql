...
begin
  ...
  if tg_table_name = 'bookings' then
    ...
    v_month_start := date_trunc('month', new.check_in::timestamp)::date;
    ...
    select count(*)
     ...
     where b.lodge_id = new.lodge_id
       ...
       and b.check_in < v_month_end;

    if v_used >= (v_booking_limit + v_booking_grace) then
      raise exception 'Monthly booking limit reached for % plan (%/%). Upgrade to % to create more bookings.',
        v_plan, v_used, (v_booking_limit + v_booking_grace), v_upgrade_plan;
    end if;
    return new;
  end if;
