-- ============================================================
-- Fix create_online_booking: replace digest() (pgcrypto) with
-- built-in md5() for the idempotency key generation.
-- digest() fails because search_path is locked to 'public'
-- and pgcrypto lives in the 'extensions' schema.
-- ============================================================
...
begin
  ...
  -- Idempotency key using built-in md5() — no extension required
  v_idem_key := coalesce(
