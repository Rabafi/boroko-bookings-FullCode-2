...
export default function RestaurantFinanceOverview() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData({
        ...
        drawer: value(1, null),
        pendingCashups: value(2, {})?.submissions || [],
      ...
      if (results.some((result) => result.status === 'rejected')) setError('Some finance signals could not be loaded. Refresh before relying on this overview.')
    } catch (loadError) {
  ...
  const summary = useMemo(() => {
    const completed = data.orders.filter((order) => !['cancelled', 'voided', 'refunded'].includes(String(order.status || '').toLowerCase()))
    const sales = completed.reduce((sum, order) => sum + Number(order.total || order.total_amount || 0), 0)
    const cash = completed.filter((order) => String(order.payment_method || '').toLowerCase() === 'cash').reduce((sum, order) => sum + Number(order.total || order.total_amount || 0), 0)
    const expenses = data.expenses.reduce((sum, row) => sum + Number(row.amount || 0), 0)
    const heldDeposits = data.deposits.filter((row) => String(row.status || 'held').toLowerCase() === 'held').reduce((sum, row) => sum + Number(row.amount || 0), 0)
    const tipsOwed = data.tips.reduce((sum, row) => sum + Number(row.available || 0), 0)
    const settlementVariance = data.settlements.reduce((sum, row) => sum + Math.abs(Number(row.variance_amount || 0)), 0)
    return { sales, cash, expenses, heldDeposits, tipsOwed, settlementVariance }
  }, [data])

  const actions = [
    ['Cash-ups & drawers', `${data.pendingCashups.length} waiting for review`, 'cashups'],
    ['Sales & payments', 'Review receipts, tenders, voids and exceptions', 'sales'],
    ['Settlements', summary.settlementVariance ? `${money(summary.settlementVariance, currency)} variance recorded today` : 'Reconcile card, mobile money and provider batches', 'settlements'],
