...
import { Edit3, LayoutGrid, MapPinned, Plus, RefreshCw, Trash2, X } from 'lucide-react'
...
export default function RestaurantTables() {
  ...
  const tablesByArea = useMemo(() => {
    ...
    for (const table of tables) {
      const area = table.area || 'Main dining'
      if (!groups.has(area)) groups.set(area, [])
      groups.get(area).push(table)
    }
    return [...groups.entries()]
  }, [tables])
  const renderTableCard = (table) => <article key={table.id} className="bb-card flex items-center justify-between gap-4 p-4"><div><p className="font-bold text-slate-900">{table.name || table.table_number}</p><p className="mt-1 text-sm text-slate-500">{table.seats || 4} seats · {table.area || 'Main dining'}</p><p className="mt-1 text-xs text-slate-400">Live status: {table.status || 'available'}</p></div>{canManageTables && <div className="flex shrink-0 gap-1"><button type="button" onClick={() => startEdit(table)} className="rounded-md p-2 text-slate-600 hover:bg-slate-100" aria-label={`Edit ${table.name || table.table_number}`}><Edit3 size={16}/></button><button type="button" onClick={() => archive(table)} disabled={saving || (table.status && table.status !== 'available')} className="rounded-md p-2 text-red-700 hover:bg-red-50 disabled:opacity-35" aria-label={`Archive ${table.name || table.table_number}`}><Trash2 size={16}/></button></div>}</article>

  return <div className="restaurant-native-page">
    <div className="restaurant-native-hero"><div><h1 className="text-2xl font-bold text-gray-900">Table setup</h1><p className="mt-1 text-sm text-gray-500">Name tables, set seats and organise service areas. Day-to-day transactions happen from the live Floor plan in the sidebar.</p></div><div className="flex gap-2"><div className="flex rounded-lg bg-gray-100 p-1" role="group" aria-label="Table layout view"><button type="button" onClick={() => setViewMode('grid')} className={`rounded-md px-3 py-1.5 text-sm font-medium ${viewMode === 'grid' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Grid</button><button type="button" onClick={() => setViewMode('area')} className={`rounded-md px-3 py-1.5 text-sm font-medium ${viewMode === 'area' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>By Area</button></div><button onClick={loadTables} className="bb-btn-outline text-sm flex items-center gap-1.5" disabled={loading}><RefreshCw size={14}/>Refresh</button>{canManageTables && <button onClick={startCreate} className="bb-btn-primary text-sm flex items-center gap-1.5"><Plus size={14}/>Add table</button>}</div></div>
    {error && <ErrorNotice className="mb-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</ErrorNotice>}{notice && <div className="mb-4 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800">{notice}</div>}
    ...
    {loading ? <div className="restaurant-native-loading"><div className="h-8 w-8 animate-spin rounded-full border-2 border-[#174c3a] border-t-transparent"/></div> : tables.length === 0 ? <div className="restaurant-native-empty"><MapPinned size={28} className="mx-auto mb-3 text-slate-400"/><p className="text-lg text-gray-600">No tables configured</p><p className="mt-1 text-sm text-gray-400">Add the first table here, then it will appear on the live Floor plan.</p></div> : viewMode === 'area' ? <div className="space-y-5">{tablesByArea.map(([area, areaTables]) => <section key={area}><h2 className="mb-2 flex items-center gap-2 text-sm font-bold text-slate-700"><LayoutGrid size={15}/> {area} · {areaTables.length} table{areaTables.length === 1 ? '' : 's'}</h2><div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">{areaTables.map((table) => renderTableCard(table))}</div></section>)}</div> : <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">{tables.map((table) => renderTableCard(table))}</div>}
