...
const isCompleted = (order) => !['voided', 'cancelled'].includes(String(order.status || '').toLowerCase())
...
export default function HposMySales() {
  ...
  return <div className="hpos-my-sales">
    ...
    {!error && <HposNotice>{message || 'Loading your PIN-verified sales…'}</HposNotice>}
    <section className="hpos-my-sales-summary">
      <div><span><CircleDollarSign size={19}/></span><small>Sales excluding tips</small><strong>{money(salesExcludingTips, currency)}</strong><p>Completed sale value before gratuities.</p></div>
      <div><span><CircleDollarSign size={19}/></span><small>Tips recorded</small><strong>{money(tips, currency)}</strong><p>Tips retained or payable under the tip policy.</p></div>
      <div><span><ReceiptText size={19}/></span><small>Receipts</small><strong>{completed.length}</strong><p>Voided receipts remain visible below.</p></div>
    ...
    <section className="hpos-my-sales-list">
      ...
      <div className="hpos-my-sales-rows">
        {visibleOrders.map((order) => <button key={order.id} type="button" onClick={() => selectOrder(order)}><div><strong>{orderLabel(order)}</strong><span>{order.table_name || order.service_mode || 'Counter'} · {order.payment_method || 'Tender not recorded'}{Number(order.tip_total || 0) > 0 ? ` · Tip ${money(order.tip_total, currency)}` : ''}</span></div><div><HposStatusBadge tone={isCompleted(order) ? 'success' : 'danger'}>{order.status || 'Completed'}</HposStatusBadge><strong>{money(order.total, currency)}</strong><ChevronRight size={18}/></div></button>)}
