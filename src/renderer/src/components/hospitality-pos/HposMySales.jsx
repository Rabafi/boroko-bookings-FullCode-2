  ...
  return <div className="hpos-my-sales">
    ...
    <section className="hpos-my-sales-list">
      ...
      <div className="hpos-my-sales-rows">
        {visibleOrders.map((order) => { const state = transactionState(order); return <button key={order.id} type="button" onClick={() => selectOrder(order)}><div><strong>{orderLabel(order)}</strong><span>{order.table_name || order.service_mode || 'Counter'} · {paymentLabel(order)}{Number(order.tip_total || 0) > 0 ? ` · Tip ${est(money(order.tip_total, currency))}` : ''}</span></div><div><HposStatusBadge tone={state === 'sale' ? 'success' : state === 'pending' ? 'warning' : 'danger'}>{stateLabel(order)}</HposStatusBadge><strong>{est(money(order.total, currency))}</strong><ChevronRight size={18}/></div></button> })}
