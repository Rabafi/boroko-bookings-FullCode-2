...
export default function HposStock() {
  ...
  const loadItems = async () => {
    ...
    try {
      ...
      setError(err?.message || 'Stock could not be refreshed.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const createItem = async () => {
    ...
    try {
      ...
      setShowCreate(false)
      setNotice('Stock item created. You can now link it to a sellable product.')
      await loadItems()
    } catch (saveError) { setError(saveError?.message || 'Could not create this stock item.') }
      ...
      <section style={{
        ...
        <div style={{ position: 'relative', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'start', gap: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 9 }}>
            ...
            {canManage && <button type="button" onClick={() => { setError(''); setShowCreate(true) }} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '10px 13px', borderRadius: 10, border: 0, color: '#33232b', background: '#f8d7a5', fontSize: 12, fontWeight: 800, cursor: 'pointer' }}><Plus size={14} /> Add stock item</button>}
          </div>
        </div>
