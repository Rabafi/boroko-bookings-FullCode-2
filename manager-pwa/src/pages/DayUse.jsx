      ...
      <div className="bg-gray-900 px-4 pt-12 pb-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-gray-400">Pool guests • {money(revenue)}</p>
          </div>
        </div>
        ...
        {entries.map((entry) => (
          <div key={entry.id} className="bg-gray-800 rounded-2xl p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-sm font-semibold text-white">{entry.guest_name}</p>
                <p className="text-xs text-gray-400 mt-1">{entry.adults} adults • {entry.children} children</p>
                <p className="text-xs text-gray-500 mt-1">{entry.payment_method || 'Cash'}</p>
              </div>
              <div className="text-right">
                <p className="text-sm font-bold text-white">{money(entry.total)}</p>
              </div>
            </div>
          </div>
        ))}
